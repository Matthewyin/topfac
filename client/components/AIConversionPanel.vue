    ...
    <v-card-text>
      ...
      <div class="mb-4">
        ...
        <v-textarea
          ...
          rows="4"
          auto-grow
          :error-messages="inputValidationMessage"
          class="natural-input"
        />
        <div class="text-caption text-medium-emphasis mt-1">
          字符数：{{ naturalInput.trim().length }} / 50000（最少10个字符）
        </div>
      </div>
...
const canConvert = computed(() => {
  return naturalInput.value.trim().length >= 10 && currentApiKey.value.trim() && !converting.value
})

// 输入验证提示
const inputValidationMessage = computed(() => {
  const trimmedInput = naturalInput.value.trim()
  if (!trimmedInput) {
    return '请输入自然语言描述'
  }
  if (trimmedInput.length < 10) {
    return `至少需要10个字符（当前${trimmedInput.length}个）`
  }
  if (trimmedInput.length > 50000) {
    return `不能超过50000个字符（当前${trimmedInput.length}个）`
  }
  return ''
})
